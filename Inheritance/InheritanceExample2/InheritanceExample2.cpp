// InheritanceExample2.cpp: определяет точку входа для консольного приложения.
//
...
#define CTORS(DC, BC) DC() { set(0); } \
                      DC(DC& f) : BC(f) { set(f); }
#define EXEC(F) double F(double r) { set(r); return F(); } \
                inline double F() { return get(); }



class Point {
	//в окремий клас винесено базові харак-
...
//в похідному класі винесено базові харак-
//теристики фігури
...
protected:
	...
	double get() { return d; }
	void set(Figure& p) { set(p.get()); Point::set(p); }
